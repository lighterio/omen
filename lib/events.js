...
var stringify = require('../common/json/stringify')
...
module.exports = Type.extend({
  ...
  'socket:connected': function (socket) {
    ...
    socket.queue.forEach(function (args) {
      ...
      var data = args[1]
      self.sent++
      socket.write(type + '~' + stringify(data) + '\n')
    })
  },
  ...
  'roster:sync': function (data, socket) {
    ...
    var isMatch = (stringify(peerRoster) === stringify(roster))
    var response = {
      name: client.name,
      isMatch: isMatch,
      add: {},
  ...
  /**
   * Fetch a value from the cache or database.
   */
  fetch: function (key, socket) {
    var self = this
    self.getValue(key, function (value) {
      socket.send('fetch:' + key, value)
    })
  },

  /**
   * Store a value in the cache and the database.
   */
  store: function (item, socket) {
    var self = this
    self.setValue(item.key, item.value, function () {
      socket.send('store:' + item.key)
    })
  },
    ...
    self.getValue(key, function (value) {
      ...
      if (value) {
        ...
        self.setValue(key, value, function () {
          socket.send('sequence:block', response)
        })
