...
var Client = module.exports = Peer.extend({
  ...
  // Number of CPUs running omen processes on this host.
  ...
  // Bootstrapping pattern, (e.g. "omen-us(east|west)-(0-99).domain.tld").
  hostPattern: os.hostname().toLowerCase(),

  // True if this process will access the ring, but not be a peer.
  isClientOnly: false,

  // Milliseconds between attempts to discover peers or measure latency.
  pingDelay: 1e3,
  ...
  init: function (options) {
    ...
    hosts.forEach(function (permutation) {
      if (permutation !== host) {
        self.addPeer(self.basePort + ':' + permutation)
      }
    })
    ...
    // Stabilize, then measure latency continually.
    self.heartbeat.index = -1
  ...
  connect: function () {
    ...
    self.server.listen(self.port, function () {
      self.log.info('[Omen] Peer ' + self.name.cyan + ' is listening.')
    })
  },
  ...
  openDatabase: function (isRetry) {
    ...
    db.open({cacheSize: 0}, function (error) {
      ...
      if (error) {
        self.setPeerStatus(Peer.DOWN)
        self.log.error('[Omen] Failed to open ' + path + '.')

        // If the folder doesn't exist, create it.
        if (!isRetry) {
          fs.readdir(location, function (error) {
            if (error) {
              self.log.log('[Omen] Attempting to create ' + path + '.')
              mkdirp(location, function () {
                self.openDatabase(1)
              })
            }
          })
        ...
        self.db = db
        self.log.info('[Omen] Opened ' + path + ' database.')
      }
    })
  },
  ...
  stabilize: function () {
    var self = this
    self.log.info('[Omen] Peer ' + self.name.cyan +
      ' stabilized'.green +
  ...
  /**
   * Send a heartbeat request to the next peer.
   */
  heartbeat: function () {
    var self = this
    var peers = self.activePeers
    var index = (self.heartbeat.index + 1) % peers.length
    var peer = peers[index]
    peer.send('heartbeat:start', {start: Date.now()})
    self.heartbeat.index = index
  },
  ...
  /**
   * Fetch a value from cache or storage, and run a callback.
   */
  ...
  /**
   * Store a value in cache and storage, and run a callback.
   */
